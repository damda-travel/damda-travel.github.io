...
- 촬영 예정 콘텐츠를 여행 동선과 연결하는 `DAMDA Stories` 영상 스틸 섹션
- 전주·무주·부안·군산을 분위기 사진으로 먼저 고르는 지역 탐색
...
- 신규 방문 시 여행 성향 진단을 자동으로 열고 여행 단계 → 국가 → 관심 테마 → 선택 연락처를 순서대로 질문합니다.
- 사용자는 `Ahora no/다음에`, 배경 영역 또는 `Esc`로 즉시 닫을 수 있으며, 완료하거나 최근에 닫은 경우 반복 노출하지 않습니다.
...
- 편집형 스크롤 모션은 요소당 한 번만 실행하며 `prefers-reduced-motion` 설정에서는 자동으로 비활성화합니다.
